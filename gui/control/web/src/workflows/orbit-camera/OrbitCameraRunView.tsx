...
export function OrbitCameraRunView({ id, initialJobId, onBack, onJob }: { id: string; initialJobId?: string; onBack: () => void; onJob: (runId: string, job: JobInfo) => void }) {
  ...
  return <main className="page detail-page">
    ...
    <header className="detail-head"><div><div className="eyebrow">VIDEOTO3D RUN</div><h1>{run.run_id}</h1><p>{String(run.source?.original_input ?? run.source?.local_file ?? '')}</p><p><strong>Capture:</strong> Orbit Camera · object fixed / camera moves · STABLE | full-RGB SfM</p></div><div className="status-row"><StatusPill value={sharedReady ? 'SHARED READY' : maskReady ? 'SPARSE PENDING' : extractReady ? 'MASK PENDING' : 'EXTRACTING'} /><StatusPill value={meshReady ? 'MESH COMPLETE' : 'MESH PENDING'} /><StatusPill value={splatReady ? 'SPLAT COMPLETE' : 'SPLAT PENDING'} /></div></header>
