...
export default function App() {
  ...
  return <div className="app-shell"><div className="ambient ambient-a" /><div className="ambient ambient-b" /><nav className="topnav"><button className="brand" onClick={back}><span className="brand-dot" />Videoto3D</button><div className="nav-actions">{activeJob && <span className="global-job-status"><span className="global-job-pulse" />{activeJob.progress?.label || activeJob.kind}<b>{activeJob.progress?.detail || 'RUNNING'}</b></span>}<span className="version">Studio V1.1.2 · Local Control</span>{exitError && <span className="nav-error">{exitError}</span>}<button className="exit-studio" onClick={exitStudio} disabled={stopping}>{stopping ? 'Stopping…' : 'Exit Studio'}</button></div></nav>{runId ? <RunDetailPage id={runId} initialJobId={jobs[runId]?.job_id} onBack={back} onJob={rememberJob} /> : <RunsPage onOpen={open} onJob={rememberJob} />}<footer>Core is the single pipeline · Control is local · Viewer stays reusable.</footer></div>
}
