...
export function RunsPage({ onOpen, onJob }: { onOpen: (id: string) => void; onJob: (runId: string, job: JobInfo) => void }) {
  ...
  return <main className="page runs-page">
    ...
    <section className="run-grid">{runs.map((run) => <button className="run-card" key={run.run_id} onClick={() => onOpen(run.run_id)}><div className="run-card-head"><div><div className="eyebrow">RUN</div><h2>{run.run_id}</h2></div><span className="open-mark">↗</span></div><div className="route-stack"><RouteCard title="Shared" subtitle={`${run.frames} frames`} status={run.shared_status} accent="shared" /><RouteCard title="Mesh Route" subtitle="OpenMVS · GLB" status={run.mesh_status} accent="mesh" /><RouteCard title="Splat Route" subtitle="Brush · PLY" status={run.splat_status} accent="splat" /></div></button>)}</section>
